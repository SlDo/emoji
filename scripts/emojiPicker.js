...
const recentlyEmojiContent = document.querySelector(
  "[data-content='recentlyEmoji']"
);
...
inputElem.addEventListener("keydown", (e) => {
  ...
  if (e.code === "ArrowUp") {
    ...
    if (!stickers.classList.contains("hidden")) {
      allEmojiContent
        .querySelector(".emoji__section")
        .querySelector(".emoji-icon")
        .focus();
    }
  }
});
...
document.addEventListener("click", (e) => {
  if (!stickers.contains(e.target) && !toggleButton.contains(e.target))
...
const createEmoji = (emoji) => {
  ...
  emojiElement.addEventListener("keydown", (e) => {
    if (e.code === "Enter") clickEmojiHandle(emoji)();
  });
...
const clickEmojiHandle = (emoji) => () => {
  ...
  const recentlyEmojiArray =
    recently != null && typeof recently === "string"
      ? JSON.parse(recently)[0].items
      : [];
  ...
  if (recentlyEmojiArray.indexOf(emoji) === -1) {
    parsed =
      recentlyEmojiArray.length >= 25
        ? [emoji, ...recentlyEmojiArray.slice(0, recentlyEmojiArray.length - 1)]
        : [emoji, ...recentlyEmojiArray];

    const emojiContent =
      recentlyEmojiContent.querySelector(".emoji__container");

    if (recentlyEmojiArray.length >= 25) {
      const emojiChildren = emojiContent.querySelectorAll(".emoji-icon");
      emojiContent.removeChild(emojiChildren[emojiChildren.length - 1]);
    }

    emojiContent.prepend(createEmoji(emoji));
...
fillEmoji(
  JSON.parse(localStorage.getItem("recentlyEmoji")),
  recentlyEmojiContent
);
